...
class Road:
    ...
    def reset(self):
        ...
        (
            self.x_fine,
            self.y_fine,
            self.left_boundary_x,
            self.left_boundary_y,
            self.right_boundary_x,
            self.right_boundary_y,
        ) = self.track

        # generate the non-translated track points
        self.track_points = np.array([(x, y) for x, y in zip(self.x_fine, self.y_fine)])
        self.left_boundary_points = np.array(
            [(x, y) for x, y in zip(self.left_boundary_x, self.left_boundary_y)]
        )
        self.right_boundary_points = np.array(
            [(x, y) for x, y in zip(self.right_boundary_x, self.right_boundary_y)]
        )
    ...
    def draw(self, screen, player_x_trans, player_y_trans):
        ...
        pygame.draw.line(
            screen, RED, self.trans_left_pts[-1], self.trans_left_pts[0], 2
        )
        pygame.draw.line(
            screen, RED, self.trans_right_pts[-1], self.trans_right_pts[0], 2
        )
    ...
    def get_initial_yaw(self):
        ...
        return np.arctan2(
            self.y_fine[1] - self.y_fine[0], self.x_fine[1] - self.x_fine[0]
        )

    def check_collision(self, car):
        # Check if the car intersects with either linestring
        return self.left_linestring.intersects(
            car.bounding_box
        ) or self.right_linestring.intersects(car.bounding_box)
