...
def load_model(algo, num = -1):
    ...
    if algo == 'DQN':
        env = make_vec_env(lambda: RacetrackEnv(render_mode='human', action_space_type='discrete'), n_envs=1)
    else:
    ...
    if algo == 'PPO':
        ...
        model = DQN.load(f"models/{algo.lower()}_racetrack_{num}.zip", env=env)
    else:
