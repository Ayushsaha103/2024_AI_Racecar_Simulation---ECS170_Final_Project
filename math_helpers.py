...
def is_point_to_left_or_right(point1, point2, given_point):
    ...
    cross_product = (
        vector_p1p2[0] * vector_p1_given[1] - vector_p1p2[1] * vector_p1_given[0]
    )
    if cross_product > 0:
        return "left"
...
def find_ey(car_pos, xy_pairs):
    ...
    if (
        is_point_to_left_or_right(closest_segment[0], closest_segment[1], car_pos)
        == "left"
    ):
        min_distance *= -1
...
def find_ephi(wp_yaw, car_yaw):
    ...
    if (
        is_point_to_left_or_right(
            (0, 0), (np.cos(wp_yaw), np.sin(wp_yaw)), (np.cos(car_yaw), np.sin(car_yaw))
        )
        == "left"
    ):
        yaw_diff *= -1
