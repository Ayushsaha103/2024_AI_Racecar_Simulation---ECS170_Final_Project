...
def check_crossed_start_line(left_boundary_x, left_boundary_y, right_boundary_x, right_boundary_y, car_x, car_y):
    start_line = [(left_boundary_x[0], left_boundary_y[0]), (right_boundary_x[0], right_boundary_y[0])]
    car_position = (car_x, car_y)
    return point_crossed_two_points(car_position, start_line)

def point_crossed_two_points(point, line, threshold=50):
    p1, p2 = line
    line_vec = np.array(p2) - np.array(p1)
    point_vec = np.array(point) - np.array(p1)
    cross_product = np.cross(line_vec, point_vec)
    return abs(cross_product) < threshold and np.dot(point_vec, line_vec) >= 0 and np.dot(point_vec, line_vec) <= np.dot(line_vec, line_vec)
